...
<p>We require an additional document for your application {{ $application->application_id }}:</p>
<p><strong>{{ $documentRequest->document_name }}</strong> ({{ $documentRequest->category }})</p>
@if($documentRequest->description)
    <p>{{ $documentRequest->description }}</p>
@endif
@if($documentRequest->deadline)
    <p>Please upload by {{ $documentRequest->deadline }}</p>
@endif
<p><a href="{{ route('track.application') }}">Upload Document</a></p>
            </div>
